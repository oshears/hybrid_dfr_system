...
typedef struct {
    ...
    XDfr_inference_uio_map maps[ MAX_UIO_MAPS ];
} XDfr_inference_uio_info;

/***************** Variable Definitions **************************************/
static XDfr_inference_uio_info uio_info;
...
static int uio_info_read_name(XDfr_inference_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDfr_inference_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDfr_inference_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDfr_inference_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDfr_inference_Initialize(XDfr_inference *InstancePtr, const char* InstanceName) {
	XDfr_inference_uio_info *InfoPtr = &uio_info;
