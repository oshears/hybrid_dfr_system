...
//------------------------Parameter----------------------
`define TV_IN_weights "../tv/cdatafile/c.dfr_inference.autotvin_weights.dat"
`define TV_OUT_weights "../tv/rtldatafile/rtl.dfr_inference.autotvout_weights.dat"
...
parameter weights_C_DATA_BITWIDTH = 32'd 32;
parameter weights_mem_depth = 32'd 100;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [weights_DATA_BITWIDTH - 1 : 0] weights_mem_0 [0: weights_mem_depth - 1]; 
reg [weights_DATA_BITWIDTH - 1 : 0] weights_mem_1 [0: weights_mem_depth - 1]; 
reg [weights_DATA_BITWIDTH - 1 : 0] weights_mem_2 [0: weights_mem_depth - 1]; 
...
initial begin : initialize_weights_mem
  integer i;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > weights_mem_depth) begin
                $display ("/home/oshears/Documents/vt/research/code/verilog/hybrid_dfr_system/vitis_hls/dfr_core/proj_dfr_core/dfr_core_solution/sim/verilog/AESL_axi_master_weights.v: Write request address %d exceed AXI master weights array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, weights_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > weights_mem_depth) begin
                $display ("/home/oshears/Documents/vt/research/code/verilog/hybrid_dfr_system/vitis_hls/dfr_core/proj_dfr_core/dfr_core_solution/sim/verilog/AESL_axi_master_weights.v: Read request address %d exceed AXI master weights array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, weights_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [weights_C_DATA_BITWIDTH - 1 : 0] token_tmp; 
